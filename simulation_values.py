...
class simulation_values():
    ...
    num_myo = int(round(0.4*carrying_capacity_myo))
    num_act = int(round(0.15*carrying_capacity_act))
    overactive_actin = False
    overactive_myosin = False
    actin_genesis_rate = .07
    myosin_genesis_rate = .09
    total_time = 250.
    time_step = float(total_time)/float(num_steps)
    decay_age = 60 #35.1
    myosin_aging = .933 # For overactive myo
